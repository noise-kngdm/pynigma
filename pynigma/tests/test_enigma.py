...
init_rotors = [
    [deepcopy(inst.ROTOR_I), deepcopy(inst.ROTOR_II), deepcopy(inst.ROTOR_III)],
    [deepcopy(inst.ROTOR_II), deepcopy(inst.ROTOR_I), deepcopy(inst.ROTOR_IV)],
    [deepcopy(inst.ROTOR_VII), deepcopy(inst.ROTOR_II), deepcopy(inst.ROTOR_IV)],
]
...
init_reflector = [
    deepcopy(inst.REFLECTOR_B),
    deepcopy(inst.REFLECTOR_C),
]
...
@pytest.mark.parametrize('rotors,positions,reflector,plugboard', [
    ([deepcopy(inst.ROTOR_I)], [0, 3, 4, 5], deepcopy(inst.REFLECTOR_B),
     init_plugboard[0]),
    (init_rotors[1]*2, init_positions[0] * 2, deepcopy(inst.REFLECTOR_C),
     init_plugboard[0]),
    (init_rotors[2], init_positions[2], deepcopy(inst.REFLECTOR_C), 1),
    ])
...
plugboard,user_input, expected_output', [
    ([16, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_I, inst.ROTOR_III, inst.ROTOR_II], 'QQP',
     PlugBoard([]), "HOLA AMIGO COMO ESTAMO".replace(' ', ''),
     "NREID YYRVA MYCJO NROR".replace(' ', '')),
    ([16, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_I, inst.ROTOR_III, inst.ROTOR_II], 'QQP',
     PlugBoard([]), "HOLA AMIGO COMO ESTAMO".replace(' ', ''),
     "NREID YYRVA MYCJO NROR".replace(' ', '')),
    ([16, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_V, inst.ROTOR_III, inst.ROTOR_II], 'QQP',
     PlugBoard([]), "hi my dear friend".replace(' ', ''),
     "AZPEA GGYZW STWG".replace(' ', '')),
    ([0, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_IV, inst.ROTOR_II, inst.ROTOR_III], 'AAA',
     PlugBoard([]), "Hola amigo como estamo".replace(' ', ''),
     "RZFGC UOBCA DGZNN SNKC".replace(' ', '')),
    ([16, 0, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_I, inst.ROTOR_II, inst.ROTOR_III, inst.ROTOR_BETA], 'AAAA',
     PlugBoard([]), "hello how are you".replace(' ', ''),
     "SFMSJ MLVKC SJAQ".replace(' ', '')),
    ([16, 9, 10, 11], inst.REFLECTOR_B, [inst.ROTOR_I, inst.ROTOR_II, inst.ROTOR_III, inst.ROTOR_BETA], 'LOLA',
     PlugBoard([]), "hello how are you".replace(' ', ''),
    ...
    ([0, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_VI, inst.ROTOR_III, inst.ROTOR_II], 'QQP',
     PlugBoard([]), "hi my dear friend".replace(' ', ''),
     "TSARA HKDYC LYVL".replace(' ', '')),
    ([25, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_VI, inst.ROTOR_III, inst.ROTOR_II], 'QQP',
     PlugBoard([]), "hi my dear friend".replace(' ', ''),
     "QSNUJ JFTWK UACW".replace(' ', '')),
    ([25, 0, 0], inst.REFLECTOR_B, [inst.ROTOR_VI, inst.ROTOR_III, inst.ROTOR_II], 'AAA',
     PlugBoard([]), "himy".replace(' ', ''),
     "QHNG".replace(' ', '')),
    # Class test
    ([25, 18, 3, 2], inst.REFLECTOR_C_thin, [inst.ROTOR_VIII, inst.ROTOR_VI, inst.ROTOR_V, inst.ROTOR_BETA], 'LEPE',

...
@pytest.mark.parametrize('rotors,positions,reflector,plugboard', [
    ([Rotors.rotor_i()], [5, 4, 3, 0], Reflectors.reflector_b(),
     init_plugboard[0]),
...
plugboard,user_input, expected_output', [
    ([0, 0, 0], Reflectors.reflector_b(), [Rotors.rotor_iv(), Rotors.rotor_iii(), test_constructor_ringstellung],
...
plugboard,user_input, expected_output', [
    ([0, 0, 16], Reflectors.reflector_b(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_i()], 'PQQ',
     PlugBoard([]), "HOLA AMIGO COMO ESTAMO".replace(' ', ''),
     "NREID YYRVA MYCJO NROR".replace(' ', '')),
    ([0, 0, 16], Reflectors.reflector_b(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_i()], 'PQQ',
     PlugBoard([]), "HOLA AMIGO COMO ESTAMO".replace(' ', ''),
     "NREID YYRVA MYCJO NROR".replace(' ', '')),
    ([0, 0, 16], Reflectors.reflector_b(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_v()], 'PQQ',
     PlugBoard([]), "hi my dear friend".replace(' ', ''),
     "AZPEA GGYZW STWG".replace(' ', '')),
    ([0, 0, 0], Reflectors.reflector_b(), [Rotors.rotor_iii(), Rotors.rotor_ii(), Rotors.rotor_iv()], 'AAA',
     PlugBoard([]), "Hola amigo como estamo".replace(' ', ''),
     "RZFGC UOBCA DGZNN SNKC".replace(' ', '')),
    ([0, 0, 0, 16], Reflectors.reflector_b(), [Rotors.rotor_beta(), Rotors.rotor_iii(), Rotors.rotor_ii(), Rotors.rotor_i()], 'AAAA', PlugBoard([]), "hello how are you".replace(' ', ''),
     "SFMSJ MLVKC SJAQ".replace(' ', '')),
    ([11, 10, 9, 16], Reflectors.reflector_b(), [Rotors.rotor_beta(), Rotors.rotor_iii(), Rotors.rotor_ii(), Rotors.rotor_i()], 'ALOL',
    ...
    ([0, 0, 0], Reflectors.reflector_b(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_vi()], 'PQQ',
     PlugBoard([]), "hi my dear friend".replace(' ', ''),
     "TSARA HKDYC LYVL".replace(' ', '')),
    ([0, 0, 25], Reflectors.reflector_b(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_vi()], 'PQQ',
     PlugBoard([]), "hi my dear friend".replace(' ', ''),
     "QSNUJ JFTWK UACW".replace(' ', '')),
    ([0, 0, 25], Reflectors.reflector_b(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_vi()], 'AAA',
     PlugBoard([]), "himy".replace(' ', ''),
     "QHNG".replace(' ', '')),
    ([0, 0, 25], Reflectors.reflector_b_thin(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_vi()], 'AAA',
    ...
    ([0, 0, 25], Reflectors.reflector_b_thin(), [Rotors.rotor_ii(), Rotors.rotor_iii(), Rotors.rotor_vii()], 'AAA',
     PlugBoard([]), "hello how are you my dear friend im fine glad you asked".replace(' ', ''),
     "NGJOY NMGRF NRCXG IIRUV YXQFI WAZIK MSFUN YJIRT QECO".replace(' ', '')),
    ([19, 2, 7], Reflectors.reflector_b_thin(), [Rotors.rotor_viii(), Rotors.rotor_vii(), Rotors.rotor_vi()], 'IOX',
     PlugBoard([]), "hello how are you my dear friend im fine glad you asked".replace(' ', ''),
     "XOWIE KNXZZ ZMCQN GZQPD KNGQQ NVATJ FMQSW IMZXH NCPA".replace(' ', '')),
    ([23, 19, 2, 7], Reflectors.reflector_b_thin(), [Rotors.rotor_gamma(), Rotors.rotor_viii(), Rotors.rotor_vii(), Rotors.rotor_vi()], 'IIOX',
     PlugBoard([]), "hello how are you my dear friend im fine glad you asked".replace(' ', ''),
     "PWJBC SEZEL JFEDS KGVXG WDJJF FLOYN IAXKH NPWOR OGYV".replace(' ', '')),
    # Class test
    ([2, 3, 18, 25], Reflectors.reflector_c_thin(), [Rotors.rotor_beta(), Rotors.rotor_v(), Rotors.rotor_vi(), Rotors.rotor_viii()], 'EPEL',
